...
| Memory 类型 | 记什么 | 适合什么场景 | 代表项目/方向 |
| --- | --- | --- | --- |
| Conversation memory | 对话 turns、摘要、session 历史 | 连续聊天、短中期上下文恢复 | OpenAI Agents sessions, LangChain |
| User profile memory | 偏好、目标、稳定约束、个人画像 | 个性化 assistant、客服、长期用户体验 | mem0, Supermemory, Memobase |
| Episodic memory | 某次任务、某次运行、某个事件和结果 | agent 复盘、debug、从经验中改进 | Memary, ReMe, Hindsight 类项目 |
| Semantic memory | 从对话/文档中抽取的长期事实 | 常识化 recall、跨 session 查询 | mem0, LangMem, Supermemory |
| Graph memory | 实体、关系、事实、时间、来源 | 事实变化、关系推理、来源追踪 | Graphiti, Cognee, Zep |
| Procedural memory | 可复用流程、技巧、纠错、策略 | AI coding、重复任务、工具使用学习 | MemOS, Hivemind 类 skill memory |
| Project memory | repo 决策、架构、约束、本地规则 | AI coding agent 和长期项目协作 | local-first / Codex-style memory |

### 2.2 Multi-agent memory 拓扑

Multi-agent memory 的关键不是“存更多”，而是 **谁拥有、谁能写、谁能读、谁负责删除和纠错**。

| 拓扑 | 说明 | 适合 | 风险 | 建议 |
| --- | --- | --- | --- | --- |
| Shared blackboard | 所有 agent 读写同一个共享记忆板 | 小团队、短任务协作 | 上下文污染、责任不清 | 只用于临时 team state |
| Private memory + shared facts | 每个 agent 有私有 scratch memory，确认后的事实进入共享记忆 | planner/coder/reviewer 组合 | promotion 规则模糊 | AI coding 默认推荐 |
| Role-scoped memory | planner、researcher、coder、reviewer 各有长期角色记忆 | 稳定角色工作流 | 角色孤岛、重复事实 | 适合固定 agent team |
| Hierarchical memory | manager agent 维护任务级记忆，subagents 保留局部 trace | 大任务拆解 | manager bottleneck | 适合 orchestrated agents |
| Federated personal memory | 多个 agent 查询用户拥有的本地 memory service | IDE、chat、assistant 共用记忆 | app 之间权限泄漏 | 适合用户可控 memory |
| Graph-backed team memory | 多 agent 共享 temporal knowledge graph | 关系复杂、事实变化频繁 | schema/query 复杂 | 简单 event memory 跑通后再上 |
| Procedural skill memory | agent 存储可复用纠错、工具序列、策略 | AI coding 和 ops 自动化 | 坏习惯持久化 | 需要人工或 policy 审批 |
...
```text
User Memory
  -> stable preferences, privacy boundaries, preferred working style

Project Memory
  -> repo facts, architecture decisions, constraints, accepted conventions

Private Agent Memory
  -> planner notes, coder run trace, reviewer critique, researcher sources

Shared Team Memory
  -> only promoted facts that multiple agents are allowed to reuse

Memory Inbox
  -> all candidate long-term memories must pass through review or policy
```

这个结构的重点是：**agent 可以观察和提议，但不要默认自动污染长期记忆**。planner、coder、reviewer 可以共享“已验证事实”，但各自的 scratchpad、推理痕迹和临时失败不应该自动变成全局事实。
